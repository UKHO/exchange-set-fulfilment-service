...
namespace UKHO.ADDS.Configuration.AACEmulator.Authentication.Hmac
{
    public class HmacHandler(IOptionsMonitor<HmacOptions> options, ILoggerFactory logger, UrlEncoder encoder) : AuthenticationHandler<HmacOptions>(options, logger, encoder)
    {
        ...
        {
            AuthenticationParameters.SignedHeaders, AuthenticationParameters.Credential, AuthenticationParameters.Signature
        };

        private IDictionary<string, IEnumerable<string>> RequiredSignedHeaders { get; } = new Dictionary<string, IEnumerable<string>>
        {
            {
                HeaderNames.Date, new List<string>
                {
                    HeaderNames.XMsDate, HeaderNames.Date
                }
            },
            {
                HeaderNames.Host, new List<string>
                {
                    HeaderNames.Host
                }
            },
            {
                HeaderNames.XMsContentSha256, new List<string>
                {
                    HeaderNames.XMsContentSha256
                }
            }
        };
