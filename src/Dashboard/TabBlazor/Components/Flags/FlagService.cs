...
namespace TabBlazor
{
    public class FlagService
    {
        public FlagService(IOptions<TablerOptions> options)
        {
            foreach (var a in options.Value.AssemblyScanFilter())
            {
                foreach (var t in a.GetTypes())
                {
                    ...
                    {
                        GeneratedFlags.Add(
                            new GeneratedFlag { Name = prop.Name, FlagType = (IFlagType)prop.GetValue(null) });
                    }
                }
            }
        ...
        private List<GeneratedFlag> GeneratedFlags { get; } = [];
        private List<IFlagType> CountryFlagTypes { get; }
        public List<GeneratedFlag> AllFlags => GeneratedFlags;


        public IFlagType GetFlagType(int numericCode) => CountryFlagTypes.FirstOrDefault(e => e.Country.Numeric == numericCode);
