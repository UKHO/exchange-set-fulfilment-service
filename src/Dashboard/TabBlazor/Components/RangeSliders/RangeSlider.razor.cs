namespace TabBlazor
{
    public partial class RangeSlider<TValue> : ComponentBase
    {
        ...
        private static string GetSliderAttribute(double value) => value.ToString().Replace(",", ".");

        private static string GetSliderAttributeGeneric(TValue value) => value.ToString().Replace(",", ".");

        private string GetMinLabel()
        {
            if (Range != null)
            {
                return LabelExpression != null ? LabelExpression(Range[minIndex]) : Range[minIndex].ToString();
            }
        ...
        private string GetMaxLabel()
        {
            if (Range != null)
            {
                return LabelExpression != null ? LabelExpression(Range[maxIndex]) : Range[maxIndex].ToString();
            }
        ...
        private void HandleRangeOnChange(ChangeEventArgs e)
        {
            ...
            if (!int.TryParse(valueString, out var newSliderValueIndex))
            {
                return;
            }
