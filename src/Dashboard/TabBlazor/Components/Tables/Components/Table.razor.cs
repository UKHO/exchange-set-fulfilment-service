...
namespace TabBlazor
{
    public class TableBase<Item> : ComponentBase, IPopupEditTable<Item>, ITable<Item>, IInlineEditTable<Item>, IDetailsTable<Item>, ITableRow<Item>, ITableState<Item>
    {
        ...
        public async Task OnDeleteItem(Item item)
        {
            ...
            {
                var result = await modalService.ShowDialogAsync(new DialogOptions { MainText = "Are you sure you want to delete?", IconType = InternalIcons.Alert_triangle, StatusColor = TablerColor.Danger });
                if (!result)
                {
                    return;
                }
        ...
        protected override void OnInitialized()
        {
            ...
            var baseAttributes = new Dictionary<string, object> { { "class", TableClass } };
        ...
        public bool IsSelected(Item item) => SelectedItems != null && SelectedItems.Contains(item);

        public bool IsEditingItem(Item item) => CurrentEditItem != null && EqualityComparer<Item>.Default.Equals(item, CurrentEditItem);

        protected bool ShowDetailsRow(Item item) => DetailsTemplate != null && IsSelected(item);
