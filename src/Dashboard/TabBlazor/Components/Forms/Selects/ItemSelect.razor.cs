...
namespace TabBlazor
{
    public partial class ItemSelect<TItem, TValue> : TablerBaseComponent, IDisposable
    {
        ...
        private readonly string[] ExpandKeys = { "Enter", " ", "ArrowDown" };
        ...
        protected override void OnAfterRender(bool firstRender)
        {
            ...
            if (fieldIdentifier is not FieldIdentifier fid)
            {
                return;
            }
        ...
        private void DropdownExpanded(bool expanded) => OnExpanded.InvokeAsync(expanded);
        ...
        {
            if (SelectedTextExpression == null)
            {
        ...
        private bool IsDisabled(TItem item) => DisabledExpression != null && DisabledExpression(item);

        private bool CanSelect() => singleSelect || MaxSelectableItems > selectedItems.Count;

        private TValue GetValue(TItem item) => ConvertExpression.Invoke(item);
