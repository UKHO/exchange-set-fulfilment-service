...
namespace TabBlazor.Components.TreeViews
{
    ...
    {
        private Dictionary<TItem, IList<TItem>> children = new();
        ...
        private bool CheckAllowDrop(TItem item)
        {
            ...
            if (Root.DraggedItem == null || Root.DraggedItem.Equals(item))
            {
                return false;
            }
        ...
        {
            if (!children.TryGetValue(item, out var value))
            {
        ...
        private string GetContainerCss(TItem item) => "d-flex align-items-center mb-2 tree-container ";


        protected bool HasChildren(TItem item) => GetChildren(item)?.Count > 0;
        ...
        private async Task DragEnd(DragEventArgs e, TItem item) => await Root.SetDroppedAsync(default, e);
