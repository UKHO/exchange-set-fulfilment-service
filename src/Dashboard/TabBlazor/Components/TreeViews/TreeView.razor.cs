...
namespace TabBlazor
{
    ...
    {
        private List<TItem> checkedItems = new();

        public TItem DraggedItem;
        private readonly List<TItem> expandedItems = new();
        ...
        public async Task ExpandAllAsync() => await ExpandAllAsync(Items);

        public void CollapseAll() => expandedItems.Clear();
        ...
        public bool? IsChecked(TItem item) => checkedItems.Contains(item);

        public bool IsExpanded(TItem item) => expandedItems.Contains(item);
