namespace TabBlazor.Components.QuickTables
{
    public struct GridItemsProviderRequest<TGridItem>
    {
        ...
        private static string ColumnNotSortableMessage<T>(ColumnBase<T> col) => $"The current sort column is of type '{col.GetType().FullName}', which does not implement {nameof(ISortBuilderColumn<TGridItem>)}, so its sorting rules cannot be applied automatically.";
    }
}
