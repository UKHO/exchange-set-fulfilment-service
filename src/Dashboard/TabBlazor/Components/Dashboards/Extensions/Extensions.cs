namespace TabBlazor.Dashboards.Extensions
{
    ...
    {
        public static DateTime StartOfYear(this DateTime date) => new(date.Year, 1, 1);
        ...
        public static DateTime StartOfWeek(this DateTime date, DayOfWeek day)
        {
            do
            {
                date = date.AddDays(-1).StartOfDay();
            } while (date.DayOfWeek != day);

            return date;
        }

        public static DateTime StartOfDay(this DateTime date) => new(date.Year, date.Month, date.Day);
        ...
        public static DateTime EndOfMonth(this DateTime date) => date.StartOfDay().AddMonths(1).AddTicks(-1);
