...
namespace TabBlazor
{
    public partial class ModalView : ComponentBase, IDisposable
    {
        ...
        public void Dispose() => ModalService.UnRegisterModalView(this);

        protected override void OnInitialized() => modalViewSettings = ModalService.RegisterModalView(this);
        ...
        public void Close() => OnClosed.InvokeAsync();

        private string GetModalStyle() => isDragged ? $"position:absolute; top: {offsetY}px; left: {offsetX}px;" : $"top: {modalViewSettings.TopOffset}px";
