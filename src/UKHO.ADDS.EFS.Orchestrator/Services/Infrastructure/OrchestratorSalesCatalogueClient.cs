...
namespace UKHO.ADDS.EFS.Orchestrator.Services.Infrastructure
{
    ...
    internal class OrchestratorSalesCatalogueClient : IOrchestratorSalesCatalogueClient
    {
        ...
        /// </remarks>
        public async Task<(S100SalesCatalogueResponse s100SalesCatalogueData, DateTime? LastModified)> GetS100ProductsFromSpecificDateAsync(DateTime? sinceDateTime, S100Build build)
        {
            ...
            var s100SalesCatalogueResult = await retryPolicy.ExecuteAsync(() =>
                _salesCatalogueClient.GetS100ProductsFromSpecificDateAsync(ScsApiVersion, ProductType, sinceDateTime, build.GetCorrelationId()));
            ...
            if (s100SalesCatalogueResult.IsSuccess(out var s100SalesCatalogueData, out var error))
            {
                ...
                switch (s100SalesCatalogueData.ResponseCode)
                {
                    ...
                    default:
                        ...
                        _logger.LogUnexpectedSalesCatalogueStatusCode(SalesCatalogUnexpectedStatusLogView.Create(build, s100SalesCatalogueData.ResponseCode));
                        return (new S100SalesCatalogueResponse(), sinceDateTime);
                }
            }

            // API call failed, log the error 
            _logger.LogSalesCatalogueApiError(error, SalesCatalogApiErrorLogView.Create(build));
