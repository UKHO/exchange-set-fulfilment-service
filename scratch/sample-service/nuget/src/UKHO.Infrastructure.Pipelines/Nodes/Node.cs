...
namespace UKHO.Infrastructure.Pipelines.Nodes
{
    ...
    public abstract class Node<T> : INode<T>
    {
        ...
        ///     Creates a new Node.
        /// </summary>
        protected Node() => _id = GetType().FullName;
        ...
        protected Node(ExecutionOptions localOptions) : this() => LocalOptions = localOptions;
        ...
        public async Task<NodeResult> ExecuteManySeriallyAsync(IEnumerable<T> subjects, ExecutionOptions options = null)
        {
            ...
            try
            {
                ...
                List<T> subjectList = subjects.ToList();
                ...
                foreach (T subject in subjectList)
                {
                    try
                    {
        ...
        public async Task<NodeResult> ExecuteManyAsync(IEnumerable<T> subjects, ExecutionOptions options = null)
        {
            ...
            try
            {
                ...
                try
                {
                    ConcurrentQueue<NodeResult> resultsQueue = new ConcurrentQueue<NodeResult>();
        ...
        public async Task<NodeResult> ExecuteAsync(T subject) => await ExecuteAsync(new ExecutionContext<T>(subject)).ConfigureAwait(false);
        ...
        public async Task<NodeResult> ExecuteAsync(IExecutionContext<T> sourceContext)
        {
            ...
            try
            {
                ...
                T subject = sourceContext.Subject;
                NodeResult result = new NodeResult(subject, Id, FlowId);
                if (!_processManyMode)
                {
                    Result = result;
                }
        ...
        protected virtual Task<NodeResultStatus> PerformExecuteAsync(IExecutionContext<T> context) => Task.FromResult(NodeResultStatus.Succeeded);
        ...
        private void ProcessExecuteManyResults(ExecutionOptions options)
        {
            ...
            List<Exception> exceptions = Result.GetFailExceptions().ToList();
