...
namespace UKHO.Infrastructure.Pipelines.Nodes
{
    ...
    {
        private readonly ConcurrentQueue<NodeResult> _childResults = new();
        ...
        public T GetSubjectAs<T>() => (T)Subject;

        /// <summary>
        ///     If the node was a failure, aggregates execptions along the failure path.
        /// </summary>
        /// <remarks>
        ///     This does not include all exceptions.  Exceptions that are descendents of nodes that are not marked as failures are
        ///     not included.
        ...
        private void AppendFailExceptions(ICollection<Exception> exceptions)
        {
            if (Status == NodeResultStatus.Failed)
            {
                ...
                foreach (NodeResult childResult in _childResults)
                {
        ...
        internal void AddChildResult(NodeResult result) => _childResults.Enqueue(result);
