...
namespace UKHO.Infrastructure.Pipelines.Tests
{
    public class WhenRunningNode
    {
        ...
        [Fact]
        public async Task Errant_Node_Run_With_ThrowOnError_True_Throws()
        {
            FaultingTestNodeA testNode = new FaultingTestNodeA();

            TestObjectA testObject = new TestObjectA();

            ExecutionContext<TestObjectA> context = new ExecutionContext<TestObjectA>(testObject) { GlobalOptions = { ThrowOnError = true } };

            await Assert.ThrowsAsync<Exception>(() => testNode.ExecuteAsync(context));
        }
        ...
        {
            SimpleTestNodeA1 testNode = new SimpleTestNodeA1(false);
