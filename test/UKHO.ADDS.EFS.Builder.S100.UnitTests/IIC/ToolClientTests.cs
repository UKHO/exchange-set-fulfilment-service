...
namespace UKHO.ADDS.EFS.Builder.S100.UnitTests.IIC
{
    ...
    public class ToolClientTests
    {
        ...
        public async Task WhenAddExchangeSetAsyncIsCalledAndSuccess_ThenReturnsSuccessResult()
        {
            ...
            var result = await _toolClient.AddExchangeSetAsync(_exchangeSetId, AuthKey, _correlationId);
            Assert.That(result.IsSuccess(out var value, out var error), Is.EqualTo(true));
            Assert.That(value?.Code, Is.EqualTo(200));
            Assert.That(_handler.LastRequestUri!.ToString(), Is.EqualTo($"{IICBaseUrl}addExchangeSet/working9/{_exchangeSetId}?authkey={AuthKey}"));
        }
        ...
        public async Task WhenAddContentAsyncIsCalledWithResourceLocationAndSuccess_ThenReturnsSuccessResult()
        {
            var response = new OperationResponse { Code = 200, Type = "Success", Message = "ok" };
            SetupHttpResponse(HttpStatusCode.OK, JsonCodec.Encode(response));
            var result = await _toolClient.AddContentAsync(ResourceLocation, _exchangeSetId, AuthKey, _correlationId);
            Assert.That(result.IsSuccess(out var value, out var error), Is.EqualTo(true));
            Assert.That(value?.Code, Is.EqualTo(200));
            Assert.That(_handler.LastRequestUri!.ToString(), Is.EqualTo($"{IICBaseUrl}addContent/working9/{_exchangeSetId}?authkey={AuthKey}&resourceLocation={ResourceLocation}"));
        }
        ...
        public async Task WhenSignExchangeSetAsyncIsCalledWithSuccess_ThenReturnsSuccess()
        {
            ...
            var result = await _toolClient.SignExchangeSetAsync(_exchangeSetId, AuthKey, _correlationId);
            ...
            Assert.That(_handler.LastRequestUri!.ToString(), Is.EqualTo($"{IICBaseUrl}signExchangeSet/working9/{_exchangeSetId}?authkey={AuthKey}"));
        }
        ...
        public async Task WhenExtractExchangeSetAsyncIsCalledWithSuccess_ThenReturnsSuccess()
        {
            ...
            var result = await _toolClient.ExtractExchangeSetAsync(_exchangeSetId, AuthKey, _correlationId, DestinationPath);
            Assert.That(result.IsSuccess(out var value, out var error), Is.EqualTo(true));
            Assert.That(value, Is.Not.Null);
            Assert.That(_handler.LastRequestUri!.ToString(), Is.EqualTo($"{IICBaseUrl}extractExchangeSet/working9/{_exchangeSetId}?authkey={AuthKey}&destination={DestinationPath}"));
        }
