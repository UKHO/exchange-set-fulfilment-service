...
namespace UKHO.ADDS.EFS.Builder.S100.UnitTests.Pipeline.Assemble
{
    ...
    internal class ProductSearchNodeTests
    {
        ...
        {
            var searchQuery = "BusinessUnit eq 'ADDS-S100' and $batch(Product Type) eq 'S-100' and (($batch(Product Name) eq '101TESTPRODUCT2' and $batch(Edition Number) eq '2' and (($batch(Update Number) eq '1' ))))";
            ...
            [
                new()
                {
                    ProductName = ProductName.From("101TestProduct2"),
                    EditionNumber = EditionNumber.From(2),
                    UpdateNumbers = [UpdateNumber.From(1)]
                }
            ];
        ...
        public async Task WhenPerformExecuteAsyncIsCalledFssReturnNextHrefUrl_ThenHandleNextHrefUrl()
        {
            ...
            using (Assert.EnterMultipleScope())
            {
                ...
                Assert.That(batchDetails!.Select(b => b.BatchId), Is.EquivalentTo(["TestBatchId1", "TestBatchId2"]));
            }
        }
